...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .scan-upload-area:hover, .scan-upload-area.dragover { border-color: var(--accent); background: rgba(124,106,247,0.07); }
  .scan-upload-icon { font-size: 48px; margin-bottom: 12px; }
  .scan-upload-title { font-size: 16px; font-weight: 700; margin-bottom: 6px; }
  .scan-upload-sub { font-size: 13px; color: var(--muted); line-height: 1.5; }
  .scan-upload-btns { display: flex; gap: 10px; margin: 0 16px 16px; }
  .scan-btn {
    flex: 1;
    padding: 13px;
    border-radius: 14px;
  ...
  .scan-btn:hover { border-color: var(--accent); background: rgba(124,106,247,0.08); }
  .scan-btn.primary {
    background: linear-gradient(135deg, var(--accent), #9d4edd);
  ...
  .scan-preview-wrap img { width: 100%; max-height: 260px; object-fit: contain; background: #111; display: block; }
  .scan-preview-remove {
  ...
  .scan-analyzing-text { font-size: 14px; color: var(--muted); margin-top: 12px; }
  .scan-result-card {
    margin: 0 16px 16px;
    background: var(--card);
    border-radius: 16px;
  ...
  .scan-result-title { font-size: 12px; font-weight: 700; color: var(--muted); text-transform: uppercase; letter-spacing: 1px; margin-bottom: 14px; }
  .scan-field { margin-bottom: 12px; }
  .scan-field-label { font-size: 11px; color: var(--muted); font-weight: 600; margin-bottom: 5px; text-transform: uppercase; letter-spacing: 0.8px; display: block; }
  .scan-field-input {
    width: 100%;
    background: var(--bg);
    border: 1px solid var(--border);
  ...
  .scan-source-grid { display: grid; grid-template-columns: repeat(3,1fr); gap: 7px; }
  .scan-source-opt {
    padding: 9px 6px;
    border-radius: 10px;
    border: 1px solid var(--border);
  ...
  .scan-source-opt.active { color: var(--accent2); border-color: var(--accent); background: rgba(124,106,247,0.12); }
  .scan-items-list { background: var(--bg); border-radius: 10px; border: 1px solid var(--border); padding: 10px 12px; font-size: 13px; color: var(--muted); line-height: 1.7; }
...
<div class="page active" id="page-home">
  ...
  <div class="quick-actions">
    ...
    <div class="qa-btn" onclick="switchPage('scan')"><span class="qa-icon">📷</span>Scan Struk</div>
  </div>
...
<div class="page" id="page-scan">
  <div style="padding: 16px 20px 8px; display:flex; justify-content:space-between; align-items:center;">
    <div style="font-size:20px; font-weight:800;">Scan Struk 📷</div>
    <div onclick="openApiKeyModal()" style="font-size:20px; cursor:pointer; padding:4px 8px;" title="API Key">⚙️</div>
  </div>
  <div style="padding: 0 20px 12px; font-size:12px; color:var(--muted);">Foto struk belanjaan, AI langsung baca & catat pengeluaran kamu otomatis.</div>
  ...
  <div id="apiKeyBanner" style="display:none; margin:0 16px 12px; padding:12px 14px; background:rgba(251,191,36,0.1); border:1px solid var(--yellow); border-radius:12px; font-size:12px; color:var(--yellow);">
    ⚠️ Belum ada API Key. Tap ⚙️ di kanan atas untuk mengisinya.
  </div>

  <!-- Upload area -->
  <div class="scan-upload-area" id="scanDropArea" onclick="document.getElementById('scanFileInput').click()">
    <div class="scan-upload-icon">🧾</div>
    <div class="scan-upload-title">Tap untuk pilih foto struk</div>
    <div class="scan-upload-sub">Dukung JPG, PNG, HEIC<br>atau seret & lepas foto ke sini</div>
  </div>
  <input type="file" id="scanFileInput" accept="image/*" capture="environment" style="display:none" onchange="onFileSelected(this)">

  <div class="scan-upload-btns">
    <button class="scan-btn" onclick="document.getElementById('scanFileInput').click()">🖼️ Galeri</button>
    <button class="scan-btn" onclick="openCamera()">📷 Kamera</button>
    <button class="scan-btn primary" id="scanAnalyzeBtn" onclick="analyzeStruk()" disabled>🔍 Analisis</button>
  </div>

  <!-- Preview foto -->
  <div class="scan-preview-wrap" id="scanPreviewWrap">
    <img id="scanPreviewImg" src="" alt="Preview struk">
    <div class="scan-preview-remove" onclick="resetScan()">✕</div>
  </div>

  <!-- Loading state -->
  <div class="scan-analyzing" id="scanAnalyzing">
    <div class="loading-dots" style="justify-content:center;">
      <div class="loading-dot"></div><div class="loading-dot"></div><div class="loading-dot"></div>
    </div>
    <div class="scan-analyzing-text">AI sedang membaca struk kamu...</div>
  </div>

  <!-- Result form -->
  <div class="scan-result-card" id="scanResultCard">
    <div class="scan-result-title">✅ Hasil Baca Struk — edit jika perlu</div>

    <div class="scan-field">
      <label class="scan-field-label">Total Belanja (Rp)</label>
      <input class="scan-field-input" id="scanAmount" type="number" inputmode="numeric" placeholder="0">
    </div>
    <div class="scan-field">
      <label class="scan-field-label">Nama Toko / Keterangan</label>
      <input class="scan-field-input" id="scanNote" type="text" placeholder="Alfamart, Indomaret, Warung...">
    </div>
    <div class="scan-field">
      <label class="scan-field-label">Kategori</label>
      <input class="scan-field-input" id="scanCategory" type="text" placeholder="Belanja, Makan, dll">
    </div>
    <div class="scan-field">
    ...
    <div class="scan-field">
      ...
      <div class="scan-items-list" id="scanItems">-</div>
    </div>

    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:8px;">
      <button class="submit-btn" style="margin-top:0;background:var(--card2);border:1px solid var(--border);" onclick="resetScan()">Ulangi</button>
      <button class="submit-btn" style="margin-top:0;" onclick="saveScanTransaction()">💾 Simpan</button>
    </div>
  </div>
</div>
...
<div class="modal-overlay" id="apiKeyOverlay" onclick="if(event.target===this)closeApiKeyModal()">
  <div class="modal">
    ...
    <div style="font-size:13px; color:var(--muted); margin-bottom:16px; line-height:1.6;">
      Masukkan API Key dari <strong>console.anthropic.com</strong> untuk mengaktifkan fitur Scan Struk. Key hanya tersimpan di browser kamu.
    </div>
...
function switchPage(page) {
  ...
  if (page === 'scan') updateApiKeyBanner();
}
...
function saveApiKey() {
  ...
  if (!key) { closeApiKeyModal(); return; }
...
function loadImageFile(file) {
  ...
  reader.onload = (e) => {
    ...
    document.getElementById('scanAnalyzeBtn').disabled = false;
    document.getElementById('scanResultCard').style.display = 'none';
    document.getElementById('scanAnalyzing').style.display = 'none';
  };
...
function resetScan() {
  ...
  document.getElementById('scanAnalyzeBtn').disabled = true;
  document.getElementById('scanResultCard').style.display = 'none';
  document.getElementById('scanAnalyzing').style.display = 'none';
  document.getElementById('scanAmount').value = '';
  document.getElementById('scanNote').value = '';
  document.getElementById('scanCategory').value = '';
  document.getElementById('scanItems').textContent = '-';
}

function selectScanSource(el) {
  document.querySelectorAll('.scan-source-opt').forEach(b => b.classList.remove('active'));
  el.classList.add('active');
  scanSelectedSource = el.dataset.src;
}

async function analyzeStruk() {
  const apiKey = localStorage.getItem('dompetku_apikey');
  if (!apiKey) { openApiKeyModal(); return; }
  if (!scanImageBase64) return;

  document.getElementById('scanAnalyzing').style.display = 'block';
  document.getElementById('scanResultCard').style.display = 'none';
  document.getElementById('scanAnalyzeBtn').disabled = true;

  const prompt = `Kamu adalah asisten pembaca struk belanja. Analisis gambar struk ini dan ekstrak informasi berikut dalam format JSON.

Kembalikan HANYA JSON ini (tanpa teks lain, tanpa backtick):
{
  "total": <angka total belanja dalam rupiah, tanpa titik atau koma, contoh: 45000>,
  "toko": "<nama toko atau merchant>",
  "kategori": "<kategori: Belanja/Makan/Transport/Kopi/Bensin/Kesehatan/Hiburan/Tagihan/Lainnya>",
  "items": "<daftar item singkat dipisah koma, max 5 item>"
}

Jika ada field yang tidak bisa dibaca, isi dengan nilai default:
- total: 0
- toko: "Toko"
- kategori: "Belanja"
- items: "-"`;
  ...
  try {
    ...
    if (data.error) {
      showInfo('Error', 'API Key bermasalah atau tidak valid. Cek lagi di ⚙️.');
      document.getElementById('scanAnalyzeBtn').disabled = false;
      return;
    }
    ...
    try { result = JSON.parse(text); } 
      ...
      result = match ? JSON.parse(match[0]) : { total: 0, toko: 'Toko', kategori: 'Belanja', items: '-' };
    }

    document.getElementById('scanAmount').value = result.total || '';
    document.getElementById('scanNote').value = result.toko || '';
    document.getElementById('scanCategory').value = result.kategori || 'Belanja';
    document.getElementById('scanItems').textContent = result.items || '-';
    document.getElementById('scanResultCard').style.display = 'block';

  } catch(e) {
    document.getElementById('scanAnalyzing').style.display = 'none';
    document.getElementById('scanAnalyzeBtn').disabled = false;
    showInfo('Koneksi Gagal', 'Gagal terhubung ke AI. Coba lagi ya!');
  }
}
...
function saveScanTransaction() {
  ...
  if (!amount || amount <= 0) {
    document.getElementById('scanAmount').focus();
    document.getElementById('scanAmount').style.borderColor = 'var(--red)';
    setTimeout(() => document.getElementById('scanAmount').style.borderColor = '', 1000);
    return;
  }
  ...
  const tx = {
    ...
    type: 'expense',
    amount,
    note: note || 'Belanja (Struk)',
    source: scanSelectedSource || 'Tunai',
  ...
  renderAll();
  switchPage('home');
  showInfo('Tersimpan! 🎉', `Pengeluaran ${formatRp(amount)} dari struk berhasil dicatat.`);
}

// Drag and drop support
document.addEventListener('DOMContentLoaded', () => {
  const drop = document.getElementById('scanDropArea');
  if (!drop) return;
